...
# 3. Default mapping nama file ke alias
default_aliases = {
    'Total.xlsx': 'Total Keseluruhan',
    'Core 1 (1).xlsx': 'Koridor 1',
    'Core 2.xlsx': 'Koridor 2',
    'Core 3.xlsx': 'Koridor 3',
    'Core 4.xlsx': 'Koridor 4',
    'Core 5.xlsx': 'Koridor 5',
    'Core 6.xlsx': 'Koridor 6',
    'Core 7 (1).xlsx': 'Koridor 7',
    'Core 8.xlsx': 'Koridor 8',
    'Core 9 (1).xlsx': 'Koridor 9',
    'Core 10.xlsx': 'Koridor 10',
    'Core 11 (1).xlsx': 'Koridor 11',
    'Core 12.xlsx': 'Koridor 12',
    'Core 13.xlsx': 'Koridor 13'
}

# 4. Cari file Excel di folder
file_list = [f for f in os.listdir(folder_path) if f.lower().endswith((".xlsx", ".xls"))]

print("Daftar file yang ditemukan:")
for f in file_list:
    print(f"- {f}")

# 5. Buat alias otomatis berdasarkan default mapping atau nama file asli
alias_dict = {}
for f in file_list:
    alias_dict[f] = default_aliases.get(f, f)  # Pakai alias kalau ada, kalau tidak pakai nama asli

# 6. Baca file Excel dan simpan ke dict dengan key alias
data_dict = {}
for original_name, alias_name in alias_dict.items():
    file_path = os.path.join(folder_path, original_name)
    try:
        df = pd.read_excel(file_path)
        data_dict[alias_name] = df
    except Exception as e:
        print(f"⚠️ Gagal membaca {original_name}: {e}")

# 7. Simpan pickle
if data_dict:
    with open("data_dict.pkl", "wb") as f:
        pickle.dump(data_dict, f)
    print(f"\n✅ {len(data_dict)} file berhasil dimuat dan disimpan dengan nama alias.")
else:
    print("⚠️ Tidak ada file Excel valid ditemukan.")
...
print("✅ Google Drive terhubung, siap dibaca Streamlit.")
